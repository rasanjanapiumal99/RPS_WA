...
Asena.addCommand({pattern: 'bashmedia ?(.*)', fromMe: wk_q, desc: bdesc, usage: 'video.mp4 && media/gif/pic.mp4'}, (async (message, match) => {    
    ...
    try {
        if (match[1].includes('jpg') || match[1].includes('tiff') || match[1].includes('raw') || match[1].includes('dng') || match[1].includes('png') || match[1].includes('jpeg')) {
            await message.client.sendMessage(id,fs.readFileSync(`/root/RPS_WA/${match[1]}`), MessageType.image, {caption: 'Made by WhatsAsena' })
        }
        else if (match[1].includes('mp4') || match[1].includes('avi') || match[1].includes('webm') || match[1].includes('mkv') || match[1].includes('mpeg')) {
            await message.client.sendMessage(id,fs.readFileSync(`/root/RPS_WA/${match[1]}`), MessageType.video, {caption: 'Made by WhatsAsena' });
        }
            ...
            await message.client.sendMessage(id,fs.readFileSync(`/root/RPS_WA/${match[1]}`), MessageType.audio);
        }
        else {
...
Asena.addCommand({pattern: 'addserver$', fromMe: wk_ad, desc: addsdesc}, (async (message, match) => {    
    if (message.reply_message.image) {
        ...
        exec('mv ' + location + ' /root/RPS_WA/server-image.' + fin)
        ...
        var fin = location.split('.')[1]
        exec('mv ' + location + ' /root/RPS_WA/server-video.' + fin)
        await message.client.sendMessage(message.jid,suc_add, MessageType.text)
    }
        ...
        var fin = location.split('.')[1]
        exec('mv ' + location + ' /root/RPS_WA/server-audio.' + fin)
        await message.client.sendMessage(message.jid,suc_add, MessageType.text)
    }
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
    if (antilink_var == 'true' && message.jid !== '94712174349-1616356915@g.us') {
...
Asena.addCommand({pattern: 'mediainfo$', fromMe: wk, desc: medinfo}, (async (message, match) => {    
    ...
    if (message.reply_message.video) {
        ...
        exec('mv ' + location + ' /root/RPS_WA/vid.mp4')
        exec('ffprobe -hide_banner -loglevel fatal -show_error -show_format -show_streams -show_programs -show_chapters -show_private_data -print_format json /root/RPS_WA/vid.mp4', async (err, st, stderr) => {
            if (err) {
